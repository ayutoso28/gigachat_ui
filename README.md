...
- **Публичная ссылка:** https://&lt;your-app&gt;.vercel.app — _замените после деплоя_
- **Скриншоты / видео:** `docs/` — _добавьте после первого прогона_
...
|------------|:------------:|-----------------------|----------|
| `VITE_GIGACHAT_AUTH_URL` | нет | `/ngw/api/v2/oauth` | Эндпоинт OAuth-сервиса. В dev резолвится через Vite-proxy, в проде — через Vercel rewrite `/ngw → /api/ngw`. |
| `VITE_GIGACHAT_API_URL` | нет | `/giga/api/v1` | Базовый URL чат-комплишенов. В dev → Vite-proxy; в проде → `/api/giga` → `https://gigachat.devices.sberbank.ru`. |
| `NODE_TLS_REJECT_UNAUTHORIZED` | **да, в проде** | — | Задаётся в настройках Vercel. Разрешает serverless-прокси обращаться к GigaChat, использующему корневой сертификат Минцифры РФ (его нет в стандартном CA-bundle Node). Влияет только на функции в `/api/*`. |
...
Конфиг: [vercel.json](vercel.json) — включает SPA-rewrite для `/chat/:id` и прокси `/ngw/*` → `/api/ngw/*`, `/giga/*` → `/api/giga/*`.

Serverless-прокси: [api/ngw/[...path].ts](api/ngw/[...path].ts) и [api/giga/[...path].ts](api/giga/[...path].ts) — пересылают запросы на реальные эндпоинты GigaChat. Нужны, потому что Vercel-edge не доверяет корневому сертификату Минцифры.
...
# 4. В дашборде Vercel → Project → Settings → Environment Variables
#    добавить:    NODE_TLS_REJECT_UNAUTHORIZED = 0
#    scope:       Production (+ Preview, если нужно тестить PR-превью)
#    После этого передеплоить: vercel --prod
```
...
- проверить DevTools → Network: запросы идут на `/ngw/...` и `/giga/...`, редиректятся в serverless (`/api/ngw/...`).

## Структура проекта

```
├── api/                        # Serverless-прокси для Vercel
│   ├── _proxy.ts               # общий transport-хелпер
│   ├── ngw/[...path].ts        # OAuth (ngw.devices.sberbank.ru:9443)
│   └── giga/[...path].ts       # Chat completions (gigachat.devices.sberbank.ru)
├── docs/
...
├── vercel.json                 # SPA-rewrite + прокси-rewrites
├── vite.config.ts              # dev-сервер + прокси /ngw и /giga
